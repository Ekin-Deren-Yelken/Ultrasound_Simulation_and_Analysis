...
$$
recieved_signal = \cos(2\pi f_{\text{doppler}}t + \phi)
$$
...
To rebuild the signal, we use only the most prominaent frequency components. This is why we did the noise reduction.
Select the frequiencies with teh largest amplitudes and model the frequencies as cosine waves using the same model as before.

$$
s_{\text{reconstructed}}(t) = \sum_{}^{} A_i \cos(2\pi f_i t + \phi)
$$

where $A_i$ is the amplitude of the detected frequency
$f_i$ is the frequency component
